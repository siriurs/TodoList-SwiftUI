        ...
        NavigationView {
            ScrollView {
                ForEach(main.todos) { todo in
                    VStack {
                        if todo.index == 0 || formatter.string(from: self.main.todos[todo.index].dueDate) != formatter.string(from: self.main.todos[todo.index - 1].dueDate) {
                            HStack {
                                Spacer()
                                    .frame(width: 30)
                                Text(date2Word(date: self.main.todos[todo.index].dueDate))
                                Spacer()
                            }
                        }
                        HStack {
                            Spacer().frame(width: 20)
                            TodoItem(main: self.main, todoIndex: .constant(todo.index))
                                .cornerRadius(10.0)
                                .clipped()
